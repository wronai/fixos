...
def diagnose_audio() -> dict[str, Any]:
    ...
    return {
        ...
        "pipewire_status": _cmd(f"systemctl --user status pipewire.service --no-pager -l 2>/dev/null | head -{MAX_AUDIO_STATUS_LINES}"),
        "pipewire_pulse_status": _cmd(f"systemctl --user status pipewire-pulse.service --no-pager -l 2>/dev/null | head -{MAX_AUDIO_STATUS_LINES}"),
        "wireplumber_status": _cmd(f"systemctl --user status wireplumber.service --no-pager -l 2>/dev/null | head -{MAX_AUDIO_STATUS_LINES}"),
        "pulseaudio_status": _cmd(f"systemctl --user status pulseaudio.service --no-pager 2>/dev/null | head -10"),
        ...
        # PipeWire objects
        "pw_dump_audio": _cmd(f"pw-dump 2>/dev/null | python3 -c \"import sys,json; d=json.load(sys.stdin); [print(n.get('info',{{}}).get('props',{{}}).get('node.name','?'), '->', n.get('info',{{}}).get('props',{{}}).get('object.path','?')) for n in d if n.get('type','') == 'PipeWire:Interface:Node']\" 2>/dev/null | head -{MAX_AUDIO_RESULTS}"),
        "pactl_info": _cmd("pactl info 2>/dev/null"),
        "pactl_sinks": _cmd(f"pactl list sinks 2>/dev/null | grep -E '(Name|State|Volume|Mute|Description)' | head -{MAX_AUDIO_RESULTS}"),
        "pactl_sources": _cmd(f"pactl list sources 2>/dev/null | grep -E '(Name|State|Volume|Mute|Description)' | head -{MAX_AUDIO_RESULTS}"),
        ...
        "kernel_audio_dmesg": _cmd(f"dmesg | grep -iE '(snd|audio|alsa|hda|sof|codec|speaker|mic|hdmi)' | tail -{MAX_AUDIO_RESULTS}"),
        "hdaudio_codec": _cmd(f"cat /proc/asound/card*/codec* 2>/dev/null | grep -E '(Codec|Address|Vendor)' | head -{MAX_AUDIO_STATUS_LINES}"),
