...
def diagnose_packages() -> dict[str, Any]:
    ...
    if IS_LINUX:
        ...
        result.update({
            ...
            "brew_cleanup_dry": _cmd("brew cleanup --dry-run 2>/dev/null | tail -20"),
...
def _diagnose_rpm_dnf() -> dict[str, Any]:
    ...
    return {
        ...
        "installed_kernels": _cmd(
            "rpm -q kernel kernel-core 2>/dev/null | sort -V"
        ),
        ...
        "devel_packages": _cmd(
            "rpm -qa 2>/dev/null | grep -E '(-devel)$' | head -30"
        ),
        "devel_packages_count": _cmd(
            "rpm -qa 2>/dev/null | grep -cE '(-devel)$'"
        ),
    }
...
def _diagnose_flatpak() -> dict[str, Any]:
    ...
    return {
        ...
        "flatpak_app_count": _cmd(
            "flatpak list --app 2>/dev/null | wc -l || echo '0'"
        ),
        "flatpak_total_size": _cmd(
            "du -sh /var/lib/flatpak 2>/dev/null || echo 'N/A'"
        ),
    }
...
def _diagnose_snap() -> dict[str, Any]:
    ...
    return {
        ...
        "snap_total_size": _cmd(
            "du -sh /snap 2>/dev/null || echo 'N/A'"
        ),
    }
