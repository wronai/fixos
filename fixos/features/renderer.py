...
class FeatureRenderer:
    ...
    def render_audit(result: AuditResult) -> None:
        ...
        console.print()
        console.print(f"  [green]✅ Zainstalowane:[/green] {installed}/{total} ({result.completion_pct:.1f}%)")
        ...
        if result.missing:
            table = Table(title="[bold red]📦 Brakujące pakiety[/bold red]", border_style="red")
    ...
    def _get_source_info(pkg: PackageInfo, distro: str) -> str:
        ...
        if pkg.flatpak:
            sources.append(f"📦 Flatpak")

        if pkg.pip:
            sources.append(f"🐍 pip")

        if pkg.cargo:
            sources.append(f"🦀 cargo")

        if pkg.install_script:
            sources.append(f"📜 script")

        return " | ".join(sources) if sources else "N/D"

    @staticmethod
    def render_package_list(packages: List[PackageInfo], title: str = "Pakiety") -> None:
    ...
    def render_system_info(system) -> None:
        ...
        console.print()
        console.print(Panel(
            "[bold cyan]🖥️  Informacje o systemie[/bold cyan]",
            border_style="cyan"
        ))

        console.print(f"  [bold]OS:[/bold]       {system.distro} {system.distro_version}")
        console.print(f"  [bold]Desktop:[/bold]  {system.desktop_env} ({system.display_server})")
        console.print(f"  [bold]GPU:[/bold]      {system.gpu_vendor} {system.gpu_model}")
        console.print(f"  [bold]PM:[/bold]       {system.pkg_manager}")
        console.print(f"  [bold]Flatpak:[/bold]  {'✅' if system.has_flatpak else '❌'}")
        console.print(f"  [bold]Pakiety:[/bold]  {len(system.installed_packages)} zainstalowanych")
        console.print()
