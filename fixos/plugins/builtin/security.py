...
class Plugin(DiagnosticPlugin):
    ...
    def _check_firewall(self) -> dict:
        ...
        ok, stdout, _, _ = run_command(
            "systemctl is-active firewalld 2>/dev/null", timeout=5
        )
        ...
        ok3, stdout3, _, _ = run_command(
            "iptables -L -n 2>/dev/null | wc -l", timeout=5
        )
        ...
        if ok and stdout:
            for line in stdout.splitlines():
                if "PermitRootLogin" in line:
                    result["root_login"] = (
                        line.split()[-1].lower() if line.split() else "unknown"
                    )
    ...
    def _check_fail2ban(self) -> dict:
        ...
        if installed:
            ok2, stdout2, _, _ = run_command(
                "systemctl is-active fail2ban 2>/dev/null", timeout=5
            )
